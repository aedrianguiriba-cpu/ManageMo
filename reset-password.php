...
// ── Look up the token (file-based) ───────────────────────────────────────────
$matched_user = null;
if ($token) {
    $tdata = _pwrLoadToken($token);
    if ($tdata) {
        $matched_user = findById(getUsers(), (int)$tdata['user_id']);
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$invalid_token) {
    ...
    if (strlen($password) < 8) {
        ...
        _pwrDeleteToken($token); // one-time use — delete immediately
